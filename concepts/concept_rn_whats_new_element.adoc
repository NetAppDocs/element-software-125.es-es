...
NOTE: Las actualizaciones acumulativas de software y firmware se instalan como parte de una actualización de Element 12.7 según la versión actual de Element que se ejecuta en un clúster de almacenamiento. Por ejemplo, si un clúster ejecuta actualmente Element 12.3.x, puede actualizar directamente a Element 12.7 para obtener las actualizaciones acumulativas de Element 12.5 y 12.7. Consulte esto para obtener información sobre las rutas de actualización admitidas https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/Element_Software/What_is_the_upgrade_matrix_for_storage_clusters_running_NetApp_Element_software["Artículo de base de conocimientos"^]
